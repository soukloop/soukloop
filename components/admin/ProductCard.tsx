...
export default function ProductCard({ product, isPending }: ProductCardProps) {
    ...
    return (
        <div className={`group rounded-xl border overflow-hidden transition-all hover:shadow-md flex flex-col h-full bg-white relative block ${isProductPending ? 'border-amber-200' : 'border-gray-100'}`}>
            {/* Image Area - Link to Product */}
            <Link href={`/admin/products/${product.id}`} className="relative aspect-square bg-gray-50 block">
            ...
            <div className="p-3 flex flex-col flex-1">
                ...
                <Link href={`/admin/products/${product.id}`} className="block mb-2">
                    <p className="font-medium text-gray-900 text-sm line-clamp-1 mb-1 group-hover:text-[#E87A3F] transition-colors" title={product.name}>{product.name}</p>
                ...
                <div className="mt-auto pt-2 border-t border-gray-50 flex items-center justify-between">
                    ...
                    >
                        ...
                        <span className="truncate max-w-[120px]">
                            {product.vendor?.businessName || product.vendor?.user?.name || 'Seller'}
                        </span>
                    </Link>
                </div>
            </div>
