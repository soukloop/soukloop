...
export default function ProductContent({ slug }: { slug?: string }) {
  ...
  const { data: occasionProducts } = useProducts({
    params: productOccasion
      ? { occasion: productOccasion, limit: 8 }
      : undefined,
  });
  ...
  const { data: locationProducts } = useProducts({
    params: productLocation
      ? { location: productLocation, limit: 8 }
      : undefined,
  });
  ...
  useEffect(() => {
    ...
    if (productId) {
      ...
      if (product) {
        // Ensure product loaded before history
        // 1. LocalStorage
        const recentlyViewedStr =
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="bg-white py-12">
        <div className="container mx-auto px-4">
          ...
          {(product as any)?.hasPendingStyle &&
            user &&
            (sellerId === user.id || isAtLeastAdmin(user.role)) && (
              <div className="mb-6 rounded-lg bg-yellow-50 p-4 text-yellow-800 border border-yellow-200 flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm mt-1">
                    This product is currently pending approval because its Dress
                    Style is under review. It is not visible to the public until
                    approved.
                  </p>
                </div>
              </div>
                  ...
                  {productImages.map((img: string, index: number) => (
                    <button
                      ...
                      className={`relative size-20 shrink-0 overflow-hidden rounded-xl border-2 transition-all ${
                        !isVideoSelected && index === selectedImage
                    ...
                    >
                      ...
                      <div className="absolute inset-0 flex items-center justify-center bg-black/30">
                        <div className="flex size-8 items-center justify-center rounded-full bg-white/90 shadow-md">
                          <svg
                            className="size-4 text-[#E87A3F] ml-0.5"
                            fill="currentColor"
                            viewBox="0 0 24 24"
                          >
              ...
              <div className="space-y-6">
                ...
                <div className="flex flex-wrap gap-2 pt-2">
                  {[
                    ...
                    .map((spec, i) => (
                      <div
                        key={i}
                        className="rounded-full bg-gray-50 border border-gray-100 px-3 py-1 text-xs font-medium text-gray-500 flex items-center gap-1"
                      >
                ...
                <div className="flex flex-col gap-3 sm:flex-row pt-4">
                  {user && sellerId === user.id ? (
                    ...
                    <>
                      <Button
                        className={`flex-1 rounded-full text-base font-semibold tracking-tight h-12 ${
                ...
                <div className="pt-4 border-t border-gray-100 mt-4">
                  ...
                  <p className="text-base leading-relaxed text-gray-600 break-words overflow-hidden">
            ...
            <div className="mt-16 rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
              <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
                ...
                <Button
                  ...
                  asChild
                >
                  <Link href={`/sellerprofile?id=${sellerId}`}>
                    View Profile
                  </Link>
                </Button>
              </div>
          ...
          {!isLoading && !error && product && (
            <div className="mt-16 space-y-12">
              ...
              {sellerProducts?.items &&
                sellerProducts.items.filter((p: any) => p.id !== productId)
                  .length > 0 && (
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <Button
                        variant="link"
                        className="text-[#E87A3F] font-bold text-sm"
                        asChild
                      >
                        <Link href={`/products?userId=${vendorUserId}`}>
                          View All
                        </Link>
                      </Button>
                    </div>
              ...
              {occasionProducts?.items &&
                productOccasion &&
                occasionProducts.items.filter((p: any) => p.id !== productId)
                  .length > 0 && (
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <h2 className="text-xl sm:text-2xl font-black tracking-tight text-gray-900">
                      ...
                      <Button
                        ...
                        asChild
                      >
                        <Link href={`/products?occasion=${productOccasion}`}>
                          View All
                        </Link>
                      </Button>
                    </div>
              ...
              {dressProducts?.items &&
                productDress &&
                dressProducts.items.filter((p: any) => p.id !== productId)
                  .length > 0 && (
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <h2 className="text-xl sm:text-2xl font-black tracking-tight text-gray-900">
                      ...
                      <Button
                        ...
                        asChild
                      >
                        <Link href={`/products?dress=${productDress}`}>
                          View All
                        </Link>
                      </Button>
                    </div>
              ...
              {brandProducts?.items &&
                productBrand &&
                brandProducts.items.filter((p: any) => p.id !== productId)
                  .length > 0 && (
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <h2 className="text-xl sm:text-2xl font-black tracking-tight text-gray-900">
                      ...
                      <Button
                        ...
                        asChild
                      >
                        <Link href={`/products?brand=${productBrand}`}>
                          View All
                        </Link>
                      </Button>
                    </div>
              ...
              {locationProducts?.items &&
                productLocation &&
                locationProducts.items.filter((p: any) => p.id !== productId)
                  .length > 0 && (
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <h2 className="text-xl sm:text-2xl font-black tracking-tight text-gray-900">
                      ...
                      <Button
                        ...
                        asChild
                      >
                        <Link href={`/products?location=${productLocation}`}>
                          View All
                        </Link>
                      </Button>
                    </div>
