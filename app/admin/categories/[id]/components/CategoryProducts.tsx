...
export default function CategoryProducts({
  ...
  const [dressStyleFilter, setDressStyleFilter] = useState(
    searchParams?.get("dressStyle") || "all",
  );
  ...
  const selectedStyleName =
    dressStyles.find((s) => s.id === dressStyleFilter)?.name || "All Styles";
          ...
          {dressStyles.length > 0 && (
            <div ref={dropdownRef} className="relative min-w-[180px]">
              <button
                ...
                className={`flex h-10 w-full items-center justify-between rounded-xl border bg-white px-3 text-sm outline-none transition-all ${
                  isStyleDropdownOpen
                    ? "border-[#E87A3F] ring-1 ring-[#E87A3F]"
              ...
              >
                ...
                <ChevronDown
                  className={`ml-2 h-4 w-4 text-gray-400 transition-transform ${isStyleDropdownOpen ? "rotate-180" : ""}`}
                />
              </button>
