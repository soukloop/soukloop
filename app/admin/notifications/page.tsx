...
import { Loader2, Bell, CheckCircle, AlertTriangle, MessageSquare, Info, Filter, Check } from 'lucide-react'
...
export default function AdminNotificationsPage() {
    ...
    const filteredNotifications = safeNotifications.filter(notification => {
        ...
        if (type === 'NEW_KYC_SUBMISSION') return canAccessRoute('/admin/sellers') || canAccessRoute('/admin/kyc')
    ...
    const getIcon = (type: string) => {
        if (type.includes('KYC')) return <CheckCircle className="w-5 h-5 text-green-500" />
        if (type.includes('REPORT')) return <AlertTriangle className="w-5 h-5 text-red-500" />
        if (type.includes('TICKET') || type.includes('MESSAGE')) return <MessageSquare className="w-5 h-5 text-blue-500" />
        ...
        <div className="max-w-4xl mx-auto space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-gray-600 mt-1">Stay updated with marketplace activity</p>
                </div>
                <div className="flex gap-3">
                    <button
                        onClick={() => setFilter(filter === 'all' ? 'unread' : 'all')}
                        className={`px-4 py-2 rounded-lg border flex items-center gap-2 ${filter === 'unread' ? 'bg-orange-50 border-orange-200 text-orange-700' : 'bg-white border-gray-300 text-gray-700'
                            }`}
                    >
                ...
                {filteredNotifications.length > 0 ? (
                    filteredNotifications.map((notification) => (
                        <div
                            ...
                            className={`p-4 hover:bg-gray-50 transition-colors cursor-pointer flex gap-4 ${!notification.isRead ? 'bg-orange-50/50' : ''
                                }`}
                        >
                            <div className={`mt-1 p-2 rounded-full ${!notification.isRead ? 'bg-white shadow-sm' : 'bg-gray-100'}`}>
