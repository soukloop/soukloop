...
export default function ChatsTab({ userId }: ChatsTabProps) {
  ...
  useEffect(() => {
    ...
    // 1) Leave previous subscription (equivalent to leave room)
    unsubscribeRef.current?.();
    unsubscribeRef.current = null;

    const convId = selectedConv.id;

    // 2) Join current conversation (subscribe)
    unsubscribeRef.current = subscribe(`conversation:${convId}`, (ctx) => {
      // Centrifuge publication data comes in ctx.data (shape depends on your backend)
      ...
      if (payload?.type === "new-message") {
        ...
        // Update active chat if matching
        ...
        // Update conversation list order/snippet for ALL messages
        setConversations((prev) => {
          const updated = prev.map((c) => {
            if (c.id === newMessage.conversationId) {
              return {
      ...
      if (payload?.type === "user-typing") {
        ...
        if (userId === current.buyerId) setIsBuyerTyping(isTyping);
        else if (userId === current.sellerId) setIsSellerTyping(isTyping);
    ...
    // 3) Cleanup = leave room (unsubscribe)
    return () => {
      unsubscribeRef.current?.();
      unsubscribeRef.current = null;
    };
  }, [selectedConv?.id, isConnected, subscribe]);
  ...
  const getOtherParticipant = (conv: Conversation): UserParticipant => {
    ...
    if (!conv.buyer || !conv.seller)
      return { id: "", name: "Unknown", image: null };
  ...
  //   };
  const getUserImage = (user: UserParticipant): string | undefined => {
    return (
      user?.profile?.avatar ?? user?.image ?? user?.vendor?.logo ?? undefined
    );
  };
          ...
          <>
            ...
            <div className="h-16 border-b bg-white px-6 flex items-center justify-between shadow-sm">
              ...
              <div className="flex items-center gap-3">
                <Link
                  href={`/product/${selectedConv.productId}`}
                  target="_blank"
                  className="relative size-10 rounded-lg bg-gray-100 overflow-hidden shrink-0 border hover:opacity-80 transition-opacity"
                >
                  {selectedConv.product.images?.[0]?.url ? (
                    <Image
                      src={selectedConv.product.images[0].url}
                      fill
                      className="object-cover"
                ...
                <div>
                  <Link
                    href={`/product/${selectedConv.productId}`}
                    target="_blank"
                    className="font-semibold text-gray-900 leading-none hover:text-orange-600 transition-colors block"
                  >
                  ...
                  <p className="text-sm font-medium text-orange-600 mt-1">
                    {new Intl.NumberFormat("en-US", {
                      style: "currency",
                      currency: "USD",
                    }).format(selectedConv.product.price)}
                  </p>
                </div>
              </div>
              ...
              {/* User Info (Right) */}
              <Link
                href={`/admin/users/${getOtherParticipant(selectedConv).id}`}
                className="flex items-center gap-3 px-3 py-1.5 rounded-lg hover:bg-gray-50 transition-colors border border-transparent hover:border-gray-200 group"
              >
