...
export default function AdminChatPage() {
  ...
  const {
    ...
    isLoading,
    mutate,
  } = useSWR(id ? `/api/admin/chats/${id}` : null, fetcher);
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribe(`conversation:${id}`, (ctx) => {
      ...
      if (data?.type === "user-typing") {
        ...
        if (userId === conversation?.buyerId)
          setIsBuyerTyping(Boolean(isTyping));
        else if (userId === conversation?.sellerId)
          setIsSellerTyping(Boolean(isTyping));
      }
    });
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            >
              <div className="text-right hidden lg:block">
                ...
                <p className="text-[10px] uppercase font-bold text-blue-500 tracking-wide">
                  Buyer
                </p>
              </div>
              <div className="h-8 w-8 rounded-full bg-blue-50 border border-blue-100 overflow-hidden">
                {getUserImage(conversation.buyer) ? (
                  <img
                    src={getUserImage(conversation.buyer)}
                    className="h-full w-full object-cover"
                    alt="Buyer"
                  />
                ) : (
            ...
            >
              <div className="h-8 w-8 rounded-full bg-orange-50 border border-orange-100 overflow-hidden order-last lg:order-first">
                {getUserImage(conversation.seller) ? (
                  <img
                    src={getUserImage(conversation.seller)}
                    className="h-full w-full object-cover"
                    alt="Seller"
                  />
                ) : (
              ...
              <div className="text-left hidden lg:block">
                ...
                <p className="text-[10px] uppercase font-bold text-orange-500 tracking-wide">
                  Seller
                </p>
              </div>
            </div>
            ...
            <div className="flex md:hidden items-center -space-x-2">
              <div
                className="h-8 w-8 rounded-full border-2 border-white bg-blue-100 overflow-hidden z-10"
                onClick={() => navigateToUser(conversation.buyerId)}
              >
                {getUserImage(conversation.buyer) ? (
                  <img
              ...
              <div
                className="h-8 w-8 rounded-full border-2 border-white bg-orange-100 overflow-hidden z-20"
                onClick={() => navigateToUser(conversation.sellerId)}
              >
                {getUserImage(conversation.seller) ? (
                  <img
            ...
            conversation.messages.map((msg: any) => {
              ...
              return (
                <div key={msg.id} className="relative">
                  ...
                  <div
                    className={`text-[10px] text-gray-400 px-2 mt-0.5 mb-2 ${isBuyer ? "text-right" : "text-left"}`}
                  >
        ...
        <div className="mx-auto max-w-3xl opacity-60 pointer-events-none select-none">
          <form className="flex items-center gap-2">
            ...
            <Button
              type="button"
              disabled
              className="size-9 bg-gray-200 text-gray-400"
            >
